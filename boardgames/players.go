...
func createPlayer(db *sqlx.DB, data PlayerModel) error {
	sql := `
	insert into tboardgameplayers (
		name
	) values (
...
func getPlayer(db *sqlx.DB, id int64) (*PlayerModel, error) {
	var retval PlayerModel
...
func getPlayerByName(db *sqlx.DB, name string) (*PlayerModel, error) {
	var retval PlayerModel
